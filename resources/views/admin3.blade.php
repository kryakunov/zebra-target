...
@if(count($works) > 0)
<div class="tab">
<div class="title">Последние задачи пользователей</div>
    <div class="content">
...
$logfile = '../storage/app/logs/'.$item->vk_id.'_'.$item->date . '.txt';
...
<div class="mywork">
    ...
    <div class="col-md-6">
        <div class="myworkheader">
            @if($item['status'] == 1)
                <a href="#" class="myworktitle">
                  {{ $item->name }}</a><span class="percent"> ({{$item->percent}}%) </span>
        ...
        <div class="myworkabout">
            ...
            <div class="time">
                <span class=workPercent>{{ ($item->live) ? 'Выполняется' : '' }}</span>
                ID: {{ $item->id }} PID: {{ $item->pids }}
            </div>
        </div>
    </div>
    <div class="col-md-3 myworkname">
        ...
        <br>
            <a href="{{ route('getworkhistory', ['id' => $item->vk_id] ) }}">История задач</a><br>
            <a href="{{ route('workrun', ['id' => $item->id] ) }}" onclick="return confirm('Вы уверены?')">Перезапустить</a><br>
            <a href="{{ route('downloadexecfile', ['id' => $item->id] ) }}">Exec output</a>
