...
<p>В исходном задании: <b>{{ $work->source_count }}</b></p>
    <textarea class="output-panel form-control mb-2"  id="textarea" rows="13">@forelse($sourceData as $value){{$value}}
@empty<p>Ничего не найдено</p>@endforelse</textarea>
    <button type="button" class="submitbutton"  onclick="copy()">Скопировать</button> <br>
    </div>
<div class="col-md-4">
...
<div class="col-md-4">
<p>В итоговом списке: <b>{{ count($data) }}</b></p>
    <textarea class="output-panel form-control mb-2"  id="textarea2" rows="13">@forelse($data as $value){{$value}}
@empty<p>Ничего не найдено</p>@endforelse</textarea>
<button type="button" class="submitbutton"  onclick="copy2()">Скопировать</button> <br>
</div>
</div>
