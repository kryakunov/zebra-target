...
class FilterGroups extends Exec
{
    public $file;
    public $id;
    public $access_token;
    ...
    public function parse($request)
    {
		...
		foreach($temp as $key => $value) 
        {
            if($continue) {
                ...
                $continue = false;
                $this->setLog('Стартуем сразу с итерации '. $key);
            }
            ...
            if (!isset($result['response']))
            {
                ...
                do {
                    if ($i > 8) {
                        $this->setLog('8 ошибок подряд. Останавливаю скрипт');
                        die;
                    }
                    ...
                    if (isset($result['error'])) {
                        ...
                        if ($result['error']['error_code'] != 29) $i++;
            ...
            if (!empty($request['date']) and isset($data)) 
            {
                ...
                {	
                    ...
                    $result = $this->vkapi('execute.getLastPostGroup', $request_params);
