...
class Execute
{
    ...
    public function __construct($id)
    {
        ...
        $this->setState('Запущено');

        $this->file = '../storage/app/'.self::FILE.'/'.$this->stream['data_to'];
        $this->logFile = '../storage/app/'.self::LOGFILE.'/'.$this->work['id'].'.txt';
        $this->sourceFile = $this->getSourceFile();//'../storage/app/'.self::SOURCEFILE.'/'.$this->stream['data_from'];
        $this->tempFile = '../storage/app/'.self::TEMPFILE.'/'.$this->work['id'].'_'.$this->stream['sample_id'] . '.txt';
        ...
        $sql = 'UPDATE sample_works SET date_start=:date_start WHERE id=:id';
        $values = array("date_start" => time(), "id" => $this->id);
        $statement = $this->PDO->prepare($sql);
        $statement->execute($values);


        $this->setLog('------------ Запущен скрипт  ' . $this->stream['name']);

    }

    public function setLastId($id)
    {
        $sql = 'UPDATE sample_works SET last_id=:last_id WHERE id=:id';
    ...
    {
        $sql = "SELECT last_id FROM sample_works WHERE id = ?";
    ...
    public function vkapi($method, $params)
    {
        ...
        {
            $error = true;
            $this->setLog('Словили исключение при запросе к vk api ');
            do {
                // Повторяем запрос
                $this->pause();
                $response = $this->tryRequest($method, $params);
    ...
    public function getToken()
    {
       ...
       $result = json_decode(file_get_contents('https://api.vk.ru/method/execute.opinionLeaders?v=5.131&access_token='.$token['token']), true);
        ...
        $sql = 'UPDATE sample_works SET token_id=:token_id WHERE id=:id';
    ...
    public function updateTokenInfo()
    {
        ...
        $statement = $this->PDO->prepare($sql);
        $statement->bindValue(1, $this->access_token);
        $statement->execute();
        $token = $statement->fetch(\PDO::FETCH_ASSOC);

        $sql = 'UPDATE date SET date=:date,requests=:requests WHERE token=:token';
        $values = array("date" => time(), "requests" => $this->countRequest, "token" => $this->access_token);
        $statement = $this->PDO->prepare($sql);
        $statement->execute($values);
    ...
    {
        $sql = "SELECT * FROM sample_works WHERE id = ?";
        ...
        $statement = $this->PDO->prepare($sql);
        $statement->bindValue(1, $res['sample_id']);
        $statement->execute();
        $res = $statement->fetch(\PDO::FETCH_ASSOC);
    ...
    public function getStreamById($id)
    {
        ...
        $sql = 'UPDATE sample_works SET date=:date WHERE id=:id';
        $values = array("date" => time(), "id" => $id);
        $statement = $this->PDO->prepare($sql);
        $statement->execute($values);
    ...
    {
        if(!isset($this->stream['data_from'])) {
            $this->setState('Не найдены исходные данные'); die;
        }
    ...
    public function getSourceFile()
    {
        ...
        // Если исходные данные из предыдущей задачи (начинается на f - первая буква слова file)
        if ($this->stream['data_from'][0] == 'f')
        {
            $path = '../storage/app/samples/' . $this->stream['data_from'];
            //$data = file_get_contents($path);

            return $path;
        }

        $dataFrom = explode('_', $this->stream['data_from']);

        // Если исходные данные из моих задач
        if ($dataFrom[0] == 0) {

            $sql = "SELECT * FROM myworks WHERE id = ?";

            $statement = $this->PDO->prepare($sql);
            $statement->bindValue(1, $dataFrom[1]);
            $statement->execute();
            $res = $statement->fetch(\PDO::FETCH_ASSOC);

            $path = '../storage/app/works/'.$res['vk_id'].'_'.$res['date'] . '.txt';
            //$data = $data = file_get_contents($path);

            return $path;

        // Если исходные данные из списков
        }elseif ($dataFrom[0] == 1) {
            ...
            $path = '../storage/app/cloud/'.$res['vk_id'].'_'.$res['track_id'] . '.txt';
           // $data = $data = file_get_contents($path);
    ...
    public function setPercent($percent)
    {
        ...
        $sql = 'UPDATE sample_works SET percent=:percent WHERE id=:id';
   ...
   {
        $sql = "SELECT status FROM sample_works WHERE id = ?";
    ...
    public function setStatus()
    {
        ...
        $sql = 'UPDATE samples SET   date_update=:date_update, date_end=:date_end WHERE id=:id';
        $values = array( "date_end" => $date_end, "date_update" => $date_end, "id" => $this->work['id']);
        $statement = $this->PDO->prepare($sql);
        $statement->execute($values);

        $sql = 'UPDATE sample_works SET status=:status, date_end=:date_end, count=:count WHERE id=:id';
        $values = array("status" => 1, "count" => $count, "date_end" => time(), "id" => $this->stream['id']);
        $statement = $this->PDO->prepare($sql);
        $statement->execute($values);

        $this->setLog('Задача выполнена');

        $this->setState('Завершено');
        $this->startNewWork();
    }

    public function setState($msg)
    {
        $sql = 'UPDATE sample_works SET state=:state WHERE id=:id';
        $values = array("state" => $msg, "id" => $this->stream['id']);
        $statement = $this->PDO->prepare($sql);
        $statement->execute($values);
    }
    ...
    {
        $sql = 'UPDATE samples SET count=:count WHERE id=:id';
    ...
    public function setLog($msg){
        ...
        $data = $time . ': stream: ' . $this->stream['sample_id'] . ' msg: ' . $msg . "\n";
    ...
    public function setError($msg)
    {
        ...
        $sql = 'UPDATE sample_works SET error=:error WHERE id=:id';
...
public function startNewWork()
{


    $sql = "SELECT * FROM sample_works WHERE sample_id = ?";
    ...
    foreach($res as $value){
        if($value['status'] !== 'wait') continue;

        $type = $this->WorkType($value['type_id']);

        $logs = 'logs/'. $this->work['id'] . '-'. $this->stream['id'] .'.txt ';

        $command = '/usr/local/php/php-8.0/bin/php ../app/Http/Execute/'.$type['type'].'.php '.escapeshellarg($value['id']);

        $process = new Process($command . ' > '.$logs.' & ');
        $pid = $process->getPid();

        $sql = 'UPDATE sample_works SET pid=:pid, error=:error WHERE id=:id';
        $values = array("pid" => $pid, "error" => null, "id" => $value['id']);
        $statement = $this->PDO->prepare($sql);
        $statement->execute($values);
    ...
    // Если эта задача была последней
    if ($end == true)
    {

        $sql = 'UPDATE samples SET date_end=:date_end, state=:state WHERE id=:id';
        $values = array("date_end" => time(), "state" => "Завершено", "id" => $this->work['id']);
        $statement = $this->PDO->prepare($sql);
        $statement->execute($values);
...
public function intersectionCounting($countMin, $groups)
{
    ...
    $groups = array_chunk($groups, 1000);
    $groups = $groups[0];
    ...
    while(count($groups) >= $countMin)
    {
        ...
        if (!file_exists($file)) {
            ...
            $this->setLog('Группа '. $minIdGroup . ' не найдена');
            continue;
        }
