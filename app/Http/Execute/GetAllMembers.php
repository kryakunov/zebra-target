...
class GetAllMembers extends Execute
{
    public $file;
    public $id;
    public $access_token;
    ...
    public function parse($request)
    {
        ...
        $this->intersectionCounting($request['min'], $groups); die;
        $lastId = null;
        ...
        foreach($groups as $key => $group) 
        {
            if ($lastId) {
                ...
                $lastId = null;
                $this->setLog('Стартую сразу с группы ' . $group);
            } 
            ...
            $this->setState('Собираю участников сообществ [' . $iter . '/'.$this->countGroups.']');
            ...
            $this->setLog('собираю участников');
        ...
        if (isset($request['min']))
            if ($request['min'] > 1) {
                ...
                $this->setState('Подсчитываю пересечения');


                    $this->intersectionCounting($request['min'], $groups);
...
public function getMembersGroup($group, $count)
{
    ...
    do {
        ...
        if (!isset($result['response']))
        {
            ...
            if ($result['error']['error_code'] == 15 or $result['error']['error_code'] == 125 or $result['error']['error_code'] == 100 or $result['error']['error_code'] == 203) {
                $msg = $result['error']['error_code'] . ' > ' . $result['error']['error_msg'];
                $this->setLog('Ошибка: ' . $msg . ' Продолжаю сkрипт ');
                unset($groups[$key]);
            }
